...
import { useManuscriptReader } from "@/hooks/use-manuscript-reader";
...
export default function StoryStudio({ privateLogin = false }: { privateLogin?: boolean }) {
  ...
  const reader = useManuscriptReader({ text: activeManuscript, documentKey: current.id + ":" + (active?.number ?? 1), textareaRef: manuscriptRef });
    ...
    <main className="min-h-screen bg-[var(--ink)] text-[var(--paper)]">
      ...
      <header className="studio-header">
        ...
        <div className="header-actions">
          <Button variant="outline" className={"reader-button" + (reader.playing ? " is-playing" : "")} aria-label={reader.playing ? "원고 듣기 · 재생 중" : reader.phase === "paused" ? "원고 듣기 · 일시정지됨" : "원고 듣기"} onClick={() => setReaderOpen(true)}><Volume2 /><span className="header-action-label">원고 듣기</span></Button>
      ...
      <Dialog open={readerOpen} onOpenChange={setReaderOpen}>
        <DialogContent className="reader-dialog sm:max-w-3xl">
          <DialogTitle className="sr-only">원고 듣기</DialogTitle>
          <DialogDescription className="sr-only">현재 회차 원고의 음성, 속도, 재생 위치를 설정하고 낭독합니다.</DialogDescription>
          <ManuscriptReader reader={reader} />
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="manual-dialog sm:max-w-4xl">
          ...
          <div className="manual-layout">
            ...
            <div className="manual-content">
              ...
              <section id="manual-write"><h3>3. 회차 집필하기</h3><p><strong>집필</strong> 탭으로 이동한 뒤 왼쪽의 회차 번호를 고릅니다. 상단의 ‘이번 화 목표’와 ‘마지막 훅’을 참고해 가운데 원고 칸에 직접 작성하세요. 원고는 저장하기 전에도 화면에서 계속 편집할 수 있습니다.</p><p><strong>목표 글자 수</strong>에 각 회차의 분량을 입력하세요. 회차 목록에서도 한 장씩 설정할 수 있으며, 집필 화면에서 현재 글자 수와 달성률을 확인합니다. 공백 포함 1~20,000자이며 비워 두면 기본 5,000자를 사용합니다. <strong>저장</strong> 또는 <strong>원고 저장</strong>을 누르면 목표도 함께 저장됩니다. AI 집필은 해당 목표를 참고하며, 전체 설계를 다시 만들거나 복원해도 회차별 목표를 유지합니다.</p><p>상단에 고정된 <strong>원고 듣기</strong> 버튼을 누르면 낭독 도구가 팝업으로 열립니다. 팝업을 닫거나 StoryWell의 다른 탭을 열어도 계속 재생하며, 같은 버튼으로 다시 열어 일시정지·정지할 수 있습니다. 작품·회차나 원고를 바꾸면 낭독을 멈춥니다. 휴대폰에서 다른 앱을 사용하거나 화면을 잠그면 브라우저 음성이 중단될 수 있습니다. 회차 전체를 읽거나 원고 입력창에 커서를 놓거나 문장을 선택한 뒤 <strong>선택한 위치부터 듣기</strong>를 누르면 그 위치부터 회차 끝까지 들을 수 있습니다. 설치된 한국어 남성 음성은 목록 위쪽에 표시되며, Microsoft Heami는 여성 음성입니다. <strong>낮은 음높이</strong>는 음높이만 조절하며 성별을 바꾸지 않습니다. 목소리·속도·톤과 작품·회차별 마지막 듣기 위치를 이 브라우저에 기억합니다. 다른 화면에 다녀오거나 새로고침한 뒤 <strong>이어 듣기</strong>를 누르세요. 원고가 바뀌면 위치를 다시 선택합니다. 듣기 기능은 원고나 음원을 별도로 업로드하지 않습니다. 온라인 음성은 브라우저·운영체제의 음성 제공업체로 원고를 전송할 수 있습니다.</p><p>긴 원고를 읽다가 처음으로 돌아가려면 화면 오른쪽 아래의 <strong>맨 위로</strong> 버튼을 누르세요. 집필 화면과 원고 입력창의 스크롤을 함께 맨 위로 이동합니다.</p><p>원고가 마무리되면 <strong>완료 표시</strong>를 누르고 <strong>원고 저장</strong>으로 확정합니다. 저장하면 해당 회차의 글자 수와 상태가 작품에 반영됩니다.</p></section>
